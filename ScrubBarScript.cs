...
public class ScrubBarScript : MonoBehaviour
{
    ...
    public void initialiseScrubBar()
    {
        ratio = 1.0f / (deserializer.getMaxSize() - 1);
    }

    public void updateScrubBar()
    {
        Vector3 oldPos = Pointer.transform.position;
        Pointer.transform.position = new Vector3(oldPos.x, oldPos.y, startOfBar + (ratio * deserializer.getFrameNum()));
    }
}
